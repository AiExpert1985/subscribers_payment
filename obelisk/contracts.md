...
An Excel or CSV file can be imported from the Accounts screen. A column is an account-number column if its header exactly matches (case-insensitive, trimmed) any alias stored in the `column_aliases` table under `section='account', field='account'`. The optional subscriber-name column is matched the same way against `field='subscriber_name'`. A file must have at least one account-number column. Rows with no parseable account numbers are silently skipped.
...
## 20260310-0000 | User-Editable Import Column Aliases via Settings | UPDATE
Old: "A column is an account-number column if its header exactly matches (case-insensitive, trimmed) any alias in the `column_aliases` table under `section='account', field='account'`. The optional subscriber-name column is matched the same way against `field='subscriber_name'`. A file must have at least one account-number column. Rows with no parseable account numbers are silently skipped."
New: A column is an account-number column if its header exactly matches (case-insensitive, trimmed) any alias stored in the `column_aliases` table under `section='account', field='account'`. The optional subscriber-name column is matched the same way against `field='subscriber_name'`. A file must have at least one account-number column. Rows with no parseable account numbers are silently skipped. Aliases are user-managed via the Settings screen; each field supports multiple aliases; required fields (account) cannot have their last alias deleted.
